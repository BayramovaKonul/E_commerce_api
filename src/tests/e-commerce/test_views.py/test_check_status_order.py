...
from tests.confest import staff, authenticated_staff, order_detail, order, address, product, category, store, user, authenticated_client


@pytest.mark.django_db
class TestOrderItemStatusView:
    def test_get_order_status(self, authenticated_staff, order_detail):
        """Test retrieving order item status."""
        url = reverse("order_item_status", args=[order_detail.id]) 
        response = authenticated_staff.get(url)

        assert response.status_code == status.HTTP_200_OK
        assert response.data == OrderDetailStatusSerializer(order_detail).data


    def test_patch_order_status(self, authenticated_staff, order_detail):
        """Test updating order item status."""
        url = reverse("order_item_status", args=[order_detail.id]) 
        new_status = {"status": "shipped"}  

        response = authenticated_staff.patch(url, data=new_status, format="json")
    ...
    def test_unauthorized_access(self, authenticated_client, order_detail):
